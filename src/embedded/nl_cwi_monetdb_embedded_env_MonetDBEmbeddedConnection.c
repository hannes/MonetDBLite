...
JNIEXPORT jobject JNICALL Java_nl_cwi_monetdb_embedded_env_MonetDBEmbeddedConnection_sendQueryInternal
    ...
    for (i = 0; i < numberOfColumns; i++) {
        ...
        (*env)->SetObjectArrayElement(env, columnNamesArray, i, (*env)->NewStringUTF(env, col.name));
        (*env)->SetObjectArrayElement(env, columnTypesArray, i, (*env)->NewStringUTF(env, nextSQLName));
        ...
        } else if(strncmp(nextSQLName, "timestamp", 9) == 0 || strncmp(nextSQLName, "timestamptz", 11) == 0) { //WARNING must come before the time!!!
