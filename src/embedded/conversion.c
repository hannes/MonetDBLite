...
PyObject *PyArrayObject_FromBAT(PyInput *inp, size_t t_start, size_t t_end, char **return_message, bool copy)
{
	...
	if (!IsBlobType(inp->bat_type) && (!IsStandardBATType(inp->bat_type) || ConvertableSQLType(inp->sql_subtype))) { // if the sql type is set, we have to do some conversion
		if (inp->scalar) {
			// FIXME: scalar SQL types
			msg = createException(MAL, "pyapi.eval", "Scalar SQL types haven't been implemented yet... sorry");
			goto wrapup;
		} else {
			BAT *ret_bat = NULL;
