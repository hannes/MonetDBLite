...
extern int mvc_export_affrows(Client c, backend *b, stream *s, lng val, str w, oid query_id);
...
extern int mvc_export_result(Client cntxt, backend *b, stream *s, int res_id);
...
extern int mvc_export_prepare(mvc *c, cq *q, res_table **result);
...
extern int mvc_result_table(Client cntxt, mvc *m, oid query_id, int nr_cols, int type, BAT *order);
